...
const SortLink: React.FC<SortLinkProps> = (props) => {
  ...
  return (
    <NavLink to={{ search: query.toString() }} className={cssClass('SortLink')}>
      ...
      {isActive && <Icon icon={sortedByIcon} color="primary100" ml="lg" />}
      {!isActive && <Icon icon="ChevronUp" color="grey40" ml="lg" />}
    </NavLink>
  );
};

const checkSortProps = (prevProps: Readonly<SortLinkProps>, nextProps: Readonly<SortLinkProps>) =>
